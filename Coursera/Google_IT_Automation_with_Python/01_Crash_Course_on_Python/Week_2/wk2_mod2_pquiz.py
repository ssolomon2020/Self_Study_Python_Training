...
# 01. This function converts miles to kilometers (km).
...
# 02. This function compares two numbers and returns them in increasing order.
...
# 04. Let's revisit our lucky_number function. We want to change it, so that instead of printing the message, it returns the message.
...
def lucky_number(name):
    number = len(name) * 9
    phrase = "Hello " + name + ". Your lucky number is " + str(number)
    return phrase
